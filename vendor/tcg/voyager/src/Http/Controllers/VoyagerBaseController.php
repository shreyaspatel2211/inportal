...
{
    // Get the slug and model for the user
    $slug = $this->getSlug($request);
    $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
    ...
    // If username is empty or null, generate it based on first_name and last_name
    if (isset($data->username) && empty($data->username)) {
        $this->generateUsername($request, $data);
    }
    ...
    // Insert and update the data fields
...
/**
 * Generate a username based on the first name and last name if it's empty.
 */
private function generateUsername(Request $request, $user)
{
    // Generate the base username from first name and last name
    $baseUsername = strtolower($request->first_name) . strtolower($request->last_name);

    // Make sure the username is unique by appending a counter if needed
    $username = $this->generateUniqueUsername($baseUsername);

    // Assign the generated username to the user
    $user->username = $username;

    // Save the user with the new username
    $user->save();
}
